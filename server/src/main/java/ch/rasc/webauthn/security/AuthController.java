...
public class AuthController {
	...
	public String registrationAdd(@AuthenticationPrincipal JooqUserDetails user) {
		...
		this.dsl.update(APP_USER)
				.set(APP_USER.REGISTRATION_ADD_START, LocalDateTime.now())
	...
	public RegistrationStartResponse registrationStart(
			...
			@RequestParam(name = "recoveryToken",
					required = false) String recoveryToken) {
		...
		if (username != null && !username.isEmpty()) {
			...
			userId = this.dsl
					...
					.values(username, LocalDateTime.now()).returning(APP_USER.ID)
					.fetchOne().getId();
			name = username;
			mode = Mode.NEW;
			...
			var record = this.dsl.select(APP_USER.ID, APP_USER.USERNAME).from(APP_USER)
					.where(APP_USER.REGISTRATION_ADD_TOKEN.eq(registrationAddTokenDecoded)
							.and(APP_USER.REGISTRATION_ADD_START
									.gt(LocalDateTime.now().minusMinutes(10))))
					.fetchOne();
			...
			this.dsl.deleteFrom(CREDENTIALS)
			.where(CREDENTIALS.APP_USER_ID.eq(userId)).execute();
			...
			PublicKeyCredentialCreationOptions credentialCreation = this.relyingParty
					.startRegistration(StartRegistrationOptions.builder()
							.user(UserIdentity.builder().name(name).displayName(name)
									.id(new ByteArray(BytesUtil.longToBytes(userId)))
									.build())
							.build());
	...
	public String registrationFinish(
			@RequestBody RegistrationFinishRequest finishRequest) {
		...
		if (startResponse != null) {
			try {
				...
				UserIdentity userIdentity = startResponse
						.getPublicKeyCredentialCreationOptions().getUser();
				...
				this.credentialRepository.addCredential(userId,
						...
						finishRequest.getCredential().getResponse()
								.getParsedAuthenticatorData().getSignatureCounter());
				...
				if (startResponse.getMode() == Mode.NEW
					...
					this.dsl.update(APP_USER)
							.set(APP_USER.REGISTRATION_START, (LocalDateTime) null)
							.set(APP_USER.RECOVERY_TOKEN, recoveryToken)
	...
	public AssertionStartResponse start(@RequestBody String username) {
		...
		AssertionStartResponse response = new AssertionStartResponse(
				Base64.getEncoder().encodeToString(assertionId),
				this.relyingParty.startAssertion(
	...
	public boolean finish(@RequestBody AssertionFinishRequest finishRequest) {
		...
		try {
			AssertionResult result = this.relyingParty

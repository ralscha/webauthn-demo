...
public class CleanupJob {
	...
	public void doCleanup() {
		...
		this.dsl.delete(APP_USER).where(
				APP_USER.REGISTRATION_START.le(LocalDateTime.now().minusMinutes(10)))
				.execute();
		...
		this.dsl.update(APP_USER)
				...
				.where(APP_USER.REGISTRATION_ADD_START
						.le(LocalDateTime.now().minusMinutes(10)))
				.execute();
	}

...
public class JooqCredentialRepository implements CredentialRepository {
	...
	public void addCredential(long userId, byte[] credentialId, byte[] publicKeyCose, long counter) {
		this.dsl.insertInto(CREDENTIALS)
		  .columns(CREDENTIALS.ID, CREDENTIALS.APP_USER_ID, CREDENTIALS.PUBLIC_KEY_COSE, CREDENTIALS.COUNT)
		  .values(credentialId, userId, publicKeyCose, counter)
		  .execute();
	}
	...
	public Set<PublicKeyCredentialDescriptor> getCredentialIdsForUsername(
		...
		var records = this.dsl.select(CREDENTIALS.ID).from(CREDENTIALS)
				.innerJoin(APP_USER).onKey().where(APP_USER.USERNAME.eq(username))
				.fetch();
		...
		for (var record : records) {
			PublicKeyCredentialDescriptor descriptor = PublicKeyCredentialDescriptor
					.builder().id(new ByteArray(record.get(CREDENTIALS.ID))).build();
	...
	public Optional<String> getUsernameForUserHandle(ByteArray userHandle) {
		System.out.println("JCR: getUsernameForUserHandle: "
	...
	public Set<RegisteredCredential> lookupAll(ByteArray credentialId) {
		...
		var records = this.dsl
				.select(CREDENTIALS.ID, CREDENTIALS.APP_USER_ID,
						CREDENTIALS.PUBLIC_KEY_COSE, CREDENTIALS.COUNT)
				.from(CREDENTIALS).where(CREDENTIALS.ID.eq(credentialId.getBytes()))
				.fetch();
		...
		for (var record : records) {
			result.add(RegisteredCredential.builder()
					...
					.userHandle(new ByteArray(
							BytesUtil.longToBytes(record.get(CREDENTIALS.APP_USER_ID))))
					.publicKeyCose(new ByteArray(record.get(CREDENTIALS.PUBLIC_KEY_COSE)))
					.signatureCount(record.get(CREDENTIALS.COUNT)).build());
		}
		return result;
	...
	public boolean updateSignatureCount(AssertionResult result) {
		...
		int noOfUpdates = this.dsl.update(CREDENTIALS)
			...
			.where(CREDENTIALS.ID.eq(result.getCredentialId().getBytes()).and(CREDENTIALS.APP_USER_ID.eq(id)))
		    .execute();
